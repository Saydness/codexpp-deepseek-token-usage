...
- 图表：柱子是圆角柱（顶端两角磨圆），费用折线平滑、线下带渐变，用量与费用的峰值会自动标出数值；
...
「账户余额」卡片显示当前余额、今日消耗、昨日消耗、本月消耗，以及每天的收盘余额 / 余额消耗 / 本机估算对照表。
...
余额只认**查到的数字**（接口返回的、历史导入的），面板不会拿本机 token 用量去推算或扣减余额——同一个 DeepSeek 账户可能多台机器共用，按本机用量算出来的余额会偏；卡片上的「今日消耗 / 本月消耗」是把当天自己的快照逐笔累加得到的，仍然是查到的值。跨零点的那一小段按小时比例分给相邻两天，所以每一天只承担属于自己的那部分；相邻快照隔得久（> 3 小时）时表里会带 `≈`，助手读到余额日志后会用真实数据替换。表格第 3 列「余额消耗」是账户级的（含账号里其他人花的），第 4 列「本机估算」才是本机 Codex 按官方费率算的钱。

不想用余额功能，把「账户余额」卡片标题行上的「启用」取消勾选即可，用量与费用统计不受影响。
