...
**当前版本（1.17.9）把面板直连查询从 UI 上撤掉了**：Codex 页面被安全策略挡住、只能借 Codex++ 的网络桥出去，而那条桥目前只放行 **POST**，DeepSeek 查余额的接口只认 **GET**，两者对不上，面板自己查必然失败。所以这块的控件和失败提示先收起来，实现代码全部保留——把脚本里的 `BRIDGE_BALANCE_QUERY_ENABLED` 改回 `true` 就会整体恢复（自动查询、Key 设置区、状态提示一起回来）。

现在余额由随 Codex 启动的本机助手自动取：它读 Codex 自己的凭据（本机中转 / 环境变量 / `%USERPROFILE%\.codex\auth.json` / 本机加密存储），用 GET 去查，只把数字交给面板，Key 不进页面也不写日志。面板上「刷新余额」会请它立刻查一次，几秒后回填；没检测到助手时提示会写明「暂时只能手动记录」。

下面是桥放开 GET 后恢复的那套 Key 来源，留档：
...
（恢复后的查询频率是：打开面板时一次，之后每 15 分钟一次，两次之间至少隔 30 秒，页面在后台时不查。）
